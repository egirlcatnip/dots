...
confirm() {
  if [ -t 0 ]; then
    read -rp "Proceed? [y/N]: " response < /dev/tty
    case "$(echo "$response" | tr '[:upper:]' '[:lower:]')" in
      y|yes) return 0;;
      *) return 1;;
    esac
  else
    log_warn "Non-interactive shell detected. Continuing in 5s..."
    sleep 5
    return 0
  fi
}
...
install_repos() {
  ...
  sudo dnf install --nogpgcheck --repofrompath 'rpmfusion-free,https://repos.rpmfusion.org/free$releasever' rpmfusion-free-release || log_warn "Failed to enable RPM Fusion Free repository"

  log_info "Enabling RPM Fusion Non-Free repository"
  sudo dnf install --nogpgcheck --repofrompath 'rpmfusion-nonfree,https://repos.rpmfusion.org/nonfree$releasever' rpmfusion-nonfree-release || log_warn "Failed to enable RPM Fusion Non-Free repository"
}
...
apply_dconf() {
  ...
  if confirm; then {
    dconf write /org/gnome/desktop/background/picture-uri "'file:///home/egirlcatnip/Repos/dotfiles/Pictures/Wallpapers/carina.png'"
    dconf write /org/gnome/desktop/background/picture-uri-dark "'file:///home/egirlcatnip/Repos/dotfiles/Pictures/Wallpapers/carina.png'"

    # Set input sources
    dconf write /org/gnome/desktop/input-sources/mru-sources "['xkb', 'sk+egirlcattype', 'xkb', 'us']"
    dconf write /org/gnome/desktop/input-sources/per-window false
    dconf write /org/gnome/desktop/input-sources/show-all-sources false
    dconf write /org/gnome/desktop/input-sources/sources "['xkb', 'sk+egirlcattype', 'xkb', 'us']"
    dconf write /org/gnome/desktop/input-sources/xkb-options "['lv3:ralt_switch']"
...
extensions() {
  ...
  if confirm; then
    cp -r ./gnome-extensions-backup ~/.local/share/gnome-shell/extensions
  else
