...
install_repos() {
  ...
  terra-release \
  || log_warn "Failed to enable Terra repository"
  ...
  || log_warn "Failed to enable RPM Fusion Free repository"
  ...
  || log_warn "Failed to enable RPM Fusion Non-Free repository"
}
...
extensions() {
  ...
  if confirm; then
    ...
    log_ok "Gnome extensions installed"
  else
