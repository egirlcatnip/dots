...
install_repos() {
  ...
  sudo dnf install -y --nogpgcheck --repofrompath 'terra,https://repos.fyralabs.com/terra$releasever' \
  terra-release || log_warn "Failed to enable Terra repository"

  log_info "Enabling RPM Fusion Free repository"
  sudo dnf install -y --nogpgcheck --repofrompath 'rpmfusion-free,https://download1.rpmfusion.org/free/fedora/releases/$releasever/Everything/$basearch/os' \
  rpmfusion-free-release || log_warn "Failed to enable RPM Fusion Free repository"

  log_info "Enabling RPM Fusion Non-Free repository"
  sudo dnf install -y --nogpgcheck --repofrompath 'rpmfusion-nonfree,https://download1.rpmfusion.org/nonfree/fedora/releases/$releasever/Everything/$basearch/os' \
  rpmfusion-nonfree-release || log_warn "Failed to enable RPM Fusion Non-Free repository"
}
