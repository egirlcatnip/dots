...
install_repos() {
  ...
  sudo dnf install -y --nogpgcheck --repofrompath 'terra,https://repos.fyralabs.com/terra$releasever' terra-release || log_warn "Failed to enable Terra repository"

  log_info "Enabling RPM Fusion Free repository"
  sudo dnf install -y --nogpgcheck --repofrompath 'rpmfusion-free,rpmfusion-nonfree,https://download1.rpmfusion.org/free/fedora/releases/$releasever/Everything/$basearch/os' rpmfusion-free-release || log_warn "Failed to enable RPM Fusion Free repository"

  log_info "Enabling RPM Fusion Non-Free repository"
  sudo dnf install -y --nogpgcheck --repofrompath 'rpmfusion-nonfree,https://download1.rpmfusion.org/nonfree/fedora/releases/$releasever/Everything/$basearch/os' rpmfusion-nonfree-release || log_warn "Failed to enable RPM Fusion Non-Free repository"
}
...
dotfiles() {
  ...
  mkdir -p "$HOME/.config"
  cp -f -r "$DOTDIR/.config/" "$HOME/.config/"

  log_ok "Dotfiles applied"
}

set_fish() {
  log_info "Set fish as default shell?"
  ...
  if confirm; then
    ...
    log_warn "Skipping fish shell setup"
  fi
}
...
extensions() {
  ...
  if confirm; then
    ...
    cp -f -r $DOTDIR/gnome-extensions-backup ~/.local/share/gnome-shell/extensions
  else
